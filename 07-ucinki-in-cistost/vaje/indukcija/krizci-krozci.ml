...
let is_full_row_2 (x1, x2, x3) = if (x1 = None || x2 = None || x3 = None) then false else true
...
let winner_of_triple_2 triple : player option =
  match triple with
  | (Some a, Some b, Some c) when (a=b && b=c) -> Some a
  | _ -> None

(* triples: (field * field * field) list     *)


let rec winner_of_list triples : player option =
  match triples with 
  | [] -> None
  | trp::rest -> match winner_of_triple trp with
    | Some pl -> Some pl
...
let show_row (a0, a1, a2) =
    "| "
  ^ String.concat " | " (List.map show_field [ a0; a1; a2 ]) (* String.concat == string.join (py) *)
  ^ " |\n"
...
let rec loop state = 
  ...
  | GameOver _ -> ()
  | OnTurn {player; grid} -> (
    let row_index = choose "vrstico" in 
    let col_index = choose "stoplec" in
    let current_field = get_field row_index col_index grid in 
      match current_field with
      | None -> 
        let new_state = place_token player grid (row_index, col_index) in 
        loop new_state
      | Some _ -> loop state 
  )
