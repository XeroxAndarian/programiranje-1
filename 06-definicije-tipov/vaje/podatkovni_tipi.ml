...
let en_evro = Euro 1.0   (* py. = Evro(1.0) *)
...
let dollar_to_euro_short (Dollar d) = Euro (d *. razmerje)
...
let rec intbool_separate_1 l = 
       let rec seperation_pomozna inti booli l = match l with
              | Empty -> (inti, booli)
              | Int(x, rep) -> seperation_pomozna (inti @ [x]) booli rep
              | Bool(x, rep) -> seperation_pomozna inti (booli @ [x]) rep
       in
       seperation_pomozna [] [] l 

let rec intbool_separate_2 l = 
       let rec seperation_pomozna inti booli l = match l with
              | Empty -> (inti, booli)
...
let update ({arcane=vrednost_arcane} as magic_counter) magic_type = match magic_type with
       | Fire -> {fire = magic_counter.fire + 1; frost = magic_counter.frost; arcane = magic_counter.arcane} (* Na dolgo *)
       | Frost -> {magic_counter with frost=(magic_counter.frost + 1)} (* z with: "vzami prejsnega s tem da popravi to" *)
       | Arcane -> { magic_counter with arcane=vrednost_arcane + 1 } (* v def. je as samo zardi te tocke, sicer bi lahko pusili samo "let update magic__counter magic_type = ..."*)
...
let rec count_magic l =
       let rec count_pomozna trenutna delovni = match delovni with
              | [] -> trenutna
              | x::rest ->
                     let trenutni' = match x.status with
                            | Newbie -> trenutna
                            | Student (magic, _) -> update trenutna magic
                            | Employed (magic, _) -> update trenutna magic
                     in
...
let rec find_candidate student_magic specialization wizard_list = match wizard_list with 
       | [] -> None
       | wizard::rest -> (
              match wizard.status with
              | Student (magic, years) when student_magic=magic -> (
                     if (zahtevana_leta specialization) <= years then 
                            Some wizard.name 
                     else (find_candidate student_magic specialization rest)
              )
              | _ -> find_candidate student_magic specialization rest
       )
