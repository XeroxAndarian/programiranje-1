...
let rec reverse sez = 
  match sez with
  | [] -> []
  | a::rep -> (reverse rep) @ [a] (* To deluje ampak je "zamudno" oz. casovno neugodno *)

let rec reverse_better sez = 
...
let je_sod = map (fun x -> x mod 2 = 0)  (* Primer simpl vpeljave *)
...
let rec mapi f sez = 
  let rec mapi_pom i sez' =
    | [] -> []
    | x::xs -> f i x :: mapi_pom (i + 1) xs   (* Na izpitu pazi na vrstni red spremenljivk*)
  in
  mapi_pom 0 sez
...
let rec zip l1 l2 =
  match (l1, l2) with
  | ([], []) -> []
  | ([], _) -> failwith "Prvi sez je prekratek"
  | (_, []) -> failwith "Drugi sez je prekratek" 
  | (x::xs, y::ys) -> (x, y) :: (zip xs ys)

let rec zip_alt_zapis l1 l2 =
  match (l1, l2) with
  | ([], []) -> []
  | ([], _)  | (_, []) -> failwith "Drugi sez je prekratek" 
  | (x::xs, y::ys) -> (x, y) :: (zip xs ys)
...
let rec unzip = function
  ...
  | (x, y)::l -> 
    ...
    let xs, ys = rekurzija in
    (*    (x::fst rekurzija, y::snd rekurzija)   ---> druga moznost vrstice 138 *)
...
let rec unzip_tlrec = 
  let rec unzip_pom (xs, ys) l = 
    match l with
    | [] -> (reverse_better xs, reverse_better ys)
    | (x, y)::tail -> unzip_pom (x:: xs, y::ys) tail
  in
  unzip_pom ([], []) l
(*----------------------------------------------------------------------------*]
...
let rec filter  f l = 
  match l with 
  | [] -> []
  | x::xs -> if f x then x::filter f xs else filter xs
...
let rec exists f = function
  ...
  | x::xs -> f x || exists f xs (* Tudi tail recursive *)
...
let rec first f default = function
  | [] -> default
  |  x::xs -> if f x then x else fist f default xs
